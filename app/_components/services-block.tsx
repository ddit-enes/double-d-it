...
import { BlockService, Service } from "@/types/block-service";
  ...
  return (
    <div className="grid grid-cols-1 gap-16 md:grid-cols-2 lg:grid-cols-3">
      {services?.map((service, index) => (
        <div
          key={index}
          className="flex flex-col h-full relative rounded-sm shadow-three hover:shadow-one dark:bg-gray-dark dark:shadow-two dark:hover:shadow-gray-dark"
        >
          ...
          <p className="mb-6 flex-1">{service.description}</p>
          {service.button_link && (
            <Button size="lg" className="text-base shadow-none w-full" asChild>
              <Link href={service.button_link}>{service.button_text}</Link>
            </Button>
          )}
        </div>
      ))}
...
const ServicesBlock = ({ service }: BlockServiceProps) => {
